...
const FiveSimPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const provider = '5sim';
        // Fetch countries directly to capture code2/iso2 if provided by backend
        const token = localStorage.getItem('auth_token') || '';
        const resp = await fetch(`${(smsApiService as any).baseUrl || 'https://api.fadsms.com/api'}/sms/countries?provider=${provider}`, {
          method: 'GET',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        });
        if (!resp.ok) throw new Error(`HTTP ${resp.status}: Failed to fetch countries`);
        const json = await resp.json();
        const list = (json?.data || []) as any[];
        if (!mounted) return;
        const mapped = list.map((row: any) => {
          const rawCode = String(row.code ?? row.id ?? row.country ?? '').toUpperCase();
          const iso2 = String(row.code2 ?? row.iso2 ?? '').toUpperCase();
          const effectiveCode = iso2 && iso2.length === 2 ? iso2 : rawCode;
          const info = smsApiService.getCountryDisplayInfo(effectiveCode);
          return { code: effectiveCode, name: info.name, flag: info.flag, provider: provider } as SmsCountry;
        });
        setCountries(mapped);
        ...
        // Normalize any non-standard cache codes (e.g., 'england' -> 'GB')
        const normalizeCountry = (code: string) => {
          const c = (code || '').toUpperCase();
          if (c === 'ENGLAND') return 'GB';
          return c;
        };
        const normalizedInitial = normalizeCountry(initialCountry);
        setSelectedCountry(normalizedInitial);

        if (normalizedInitial) {
          const svc = await smsApiService.getServices(normalizedInitial, provider);
          if (!mounted) return;
          setServices(svc);
          // Prefer WhatsApp default if present
        ...
        setError(e?.message || 'Failed to load Fadded SIM countries/services');
      } finally {
  ...
  const onCountryChange = async (countryCode: string) => {
    try {
      ...
      setLoading(true);
      const svc = await smsApiService.getServices(normalized, '5sim');
      setServices(svc);
      const wa = svc.find(s => (s.service || '').toLowerCase().includes('wa') || (s.name || '').toLowerCase().includes('whatsapp'));
      setSelectedService(wa?.service || svc[0]?.service || '');
    } catch (e: any) {
  ...
  const createOrder = async () => {
    try {
      setCreating(true);
      setError('');
      toast.loading('Creating Fadded SIM order…', { id: 'create-order' });
      const order = await smsApiService.createOrder(selectedCountry, selectedService, 'manual', '5sim');
  ...
  return (
    <div className={`min-h-screen p-4 pb-24 space-y-6 transition-colors duration-200 ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      <div className="mb-2">
        <h1 className={`${isDark ? 'text-white' : 'text-gray-900'} text-lg font-semibold`}>Fadded SIM</h1>
      </div>
      ...
      <div className={`${isDark ? 'text-gray-400' : 'text-gray-600'} text-xs`}>Route: /faddedsim</div>
