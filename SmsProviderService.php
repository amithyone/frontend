...
class SmsProviderService
{
    ...
    {
        // TextVerified API v2: GET /countries
        $url = 'https://www.textverified.com/api/v2/countries';
        $headers = [
            'Authorization' => 'Basic ' . base64_encode(($config['settings']['username'] ?? '') . ':' . ($config['api_key'] ?? '')),
        ...
        if (!$response->successful()) {
            Log::error('TextVerified getCountries HTTP', [
                'url' => $url,
                'status' => $response->status(),
        ...
        // TextVerified is US-only service, so return US only
        return [
            [
                'code' => 'US',
                'name' => 'United States'
